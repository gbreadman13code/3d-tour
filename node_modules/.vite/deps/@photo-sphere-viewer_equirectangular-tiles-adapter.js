import {
  ...
  events_exports,
  utils_exports
} from "./chunk-LBSL2FJQ.js";

// node_modules/@photo-sphere-viewer/equirectangular-tiles-adapter/index.module.js
var Task = class {
...
var Queue = class {
  ...
  start() {
    ...
    const nextTask = Object.values(this.tasks).filter(
      (task) => task.status === 1
      /* PENDING */
...
function buildErrorMaterial() {
  ...
  return new MeshBasicMaterial({ map: utils_exports.createTexture(canvas) });
}
  ...
  if (isMultiTiles(panorama)) {
    panorama.levels.forEach((level) => {
      ...
      if ("zoomRange" in level) {
        utils_exports.logWarn('EquirectangularTilesAdapter: "zoomRange" property is deprecated and must be removed');
      }
    });
...
var _EquirectangularTilesAdapter = class _EquirectangularTilesAdapter2 extends AbstractAdapter {
  ...
  static withConfig(config) {
    return [_EquirectangularTilesAdapter2, config];
  }
  init() {
    super.init();
    this.viewer.addEventListener(events_exports.TransitionDoneEvent.type, this);
    this.viewer.addEventListener(events_exports.PositionUpdatedEvent.type, this);
    this.viewer.addEventListener(events_exports.ZoomUpdatedEvent.type, this);
  }
  destroy() {
    var _a, _b, _c;
    this.viewer.removeEventListener(events_exports.TransitionDoneEvent.type, this);
    this.viewer.removeEventListener(events_exports.PositionUpdatedEvent.type, this);
    this.viewer.removeEventListener(events_exports.ZoomUpdatedEvent.type, this);
    this.__cleanup();
    (_b = (_a = this.state.errorMaterial) == null ? void 0 : _a.map) == null ? void 0 : _b.dispose();
    (_c = this.state.errorMaterial) == null ? void 0 : _c.dispose();
  ...
  handleEvent(e) {
    switch (e.type) {
      ...
      case events_exports.ZoomUpdatedEvent.type:
        this.__refresh();
        break;
      case events_exports.TransitionDoneEvent.type:
  ...
  createMesh(panoData) {
    ...
    const geometry = new SphereGeometry(
      constants_exports.SPHERE_RADIUS,
  ...
  disposeMesh(group) {
    ...
    tilesMesh.material.forEach((m) => {
      var _a;
      (_a = m.map) == null ? void 0 : _a.dispose();
      m.dispose();
    });
  }
  /**
  ...
  __loadTile(tile, task) {
    return this.viewer.textureLoader.loadImage(tile.url, null, this.viewer.state.textureData.cacheKey).then((image) => {
      if (!task.isCancelled()) {
        ...
        const material = new MeshBasicMaterial({ map: utils_exports.createTexture(image, mipmaps) });
        this.__swapMaterial(tile, material, false);
        this.viewer.needsUpdate();
      }
    }).catch((err) => {
      if (!utils_exports.isAbortError(err) && !task.isCancelled() && this.config.showErrorTile) {
...
@photo-sphere-viewer/equirectangular-tiles-adapter/index.module.js:
  (*!
   ...
   *)
*/
//# sourceMappingURL=@photo-sphere-viewer_equirectangular-tiles-adapter.js.map
